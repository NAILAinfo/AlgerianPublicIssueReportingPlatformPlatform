...
export async function mockGetReports(): Promise<{ reports: MockReport[] }> {
...
export async function mockUpdateReportStatus(
  reportId: string, 
  status: 'new' | 'in-progress' | 'resolved'
): Promise<{ message: string; report: MockReport }> {
...
export async function mockGetStatistics(): Promise<{
  statistics: {
    totalReports: number;
    ...
    monthlyData: Record<string, { total: number; resolved: number }>;
    lastUpdated: string;
  }
}> {
  ...
  const monthlyData: Record<string, { total: number; resolved: number }> = {};
  mockReports.forEach(report => {
